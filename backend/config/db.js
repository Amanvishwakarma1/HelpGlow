...
const initDb = async () => {
    try {
        ...
        // 5. Create campaign_comments table
        await pool.query(`
        ...
        // 8. Admin Account Synchronization (Salting & Hashing)
        const adminEmail = process.env.ADMIN_EMAIL;
        const adminPassword = process.env.ADMIN_PASSWORD;
        const adminUsername = process.env.ADMIN_USERNAME || 'HelpGlowAdmin';
        ...
        if (adminEmail && adminPassword) {
            ...
            if (checkAdmin.rows.length > 0) {
                ...
                console.log('✅ Admin credentials updated and synchronized with salted bcrypt hash');
            } else {
                await pool.query(
                    'INSERT INTO users (username, email, password_hash, role) VALUES ($1, $2, $3, $4)',
                    [adminUsername, adminEmail, hashedPassword, 'admin']
                );
                console.log('✅ Admin account created with salted bcrypt hash');
            }
        }
